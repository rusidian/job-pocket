...
`user_info` 배열의 인덱스 의미는 다음과 같다: `[0]` username, `[1]` password hash, `[2]` email, `[3]` reset_token, `[4]` resume_data (JSON 문자열).
...
### 3.3 `POST /api/auth/reset-pw`

비밀번호 재설정. 현재는 이메일 인증 없이 바로 변경되며, 프론트엔드에서 코드 검증을 마친 후에만 호출하는 것을 전제로 한다.
